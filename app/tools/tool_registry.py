...
from app.tools.base import BaseTool, FunctionTool, tool
...
class ToolRegistry:
    ...
    RESERVED_NAMES = {
        ...
        "api"  # Add 'api' as reserved for /api/* routes
    }
    RESERVED_PATTERN = re.compile(r"^(auth|openapi|schema|session|docs|redoc|health|api)(\/.*)?$")
    ...
    def discover_tools(self, path: Union[str, Path], strict: bool = False) -> List[Dict[str, Any]]:
        ...
        crawl_env = os.getenv("CRAWL_ENVIRONMENT", "local")
        logger.info(f"Tool discovery running in '{crawl_env}' mode (strict={strict}).")
