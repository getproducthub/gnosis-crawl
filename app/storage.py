...
class CrawlStorageService:
    ...
    def _ensure_local_dirs(self):
        ...
        for directory in base_dirs:
            try:
                ...
                raise Exception(f"Cannot create storage directory {directory}: {e}")
            except Exception as e:
                logger.error(f"Failed to create directory {directory}: {e}")
                raise Exception(f"Directory creation failed: {e}")
    ...
    async def save_json(self, data: Dict[str, Any], filename: str, session_id: str):
        """Save JSON data to storage"""
        content = json.dumps(data, indent=2)
        await self.save_file(content, filename, session_id)
    ...
    async def _save_local_file(self, file_path: str, content: bytes):
        ...
        try:
            ...
            logger.debug(f"Created directory: {full_path.parent}")
        except PermissionError as e:
            logger.error(f"Permission denied creating directory {full_path.parent}: {e}")
            raise Exception(f"Cannot create storage directory {full_path.parent}: {e}")
        except Exception as e:
            logger.error(f"Failed to create directory {full_path.parent}: {e}")
            raise Exception(f"Directory creation failed: {e}")
        ...
        try:
            ...
            raise Exception(f"Cannot write to file {full_path}: {e}")
