...
async def list_session_files(session_id: str, prefix: Optional[str] = None, user_email: str = Depends(get_user_email)):
    """List files stored for a session. Optional prefix (e.g., 'results', 'screenshots')."""
    storage = CrawlStorageService(user_email)
    try:
        pref = prefix or ''
...
async def get_session_file(session_id: str, path: str, user_email: str = Depends(get_user_email)):
